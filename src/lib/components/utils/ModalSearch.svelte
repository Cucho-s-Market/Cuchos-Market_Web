...
<label bind:this={modalOutside} for="my-modal-5" class="modal opacity-100 border:none flex items-start visible pointer-events-auto" transition:fly="{{ duration: 700 }}">
	<label class="absolute w-[400px]" for="">
		<!-- MAIN CONTENT -->
		<div class="w-full">
			<form>   
				<div class="relative h-full mt-[20px] ">
					<label for="search"></label>
					<input id="default-search" bind:value={search} on:input={handle_search} class="w-full border border-primary block p-4 pl-5 text-sm text-primary-900 rounded-lg bg-grey-50" placeholder="Search" />
